...
**v1.9.30 eklendi (2026-08-26, kullanıcı isteği — izlenen walkthrough + zorluğa bağlı board'lar):**
- **Rehberler artık video:** `BoardGuide.jsx` altı board için de kendi kendine dönen, İZLENEN bir simülasyon çalıştırır — piksel imleç (`Cursor`/`cursorAt` keyframe interpolasyonu) doğru anda doğru yere basar. Sabotajda amber pencerede CUT CURRENT'a, kilitte veriş noktasında DURMAYA (buton yok), kronolojide ▲+SUBMIT'e, çelişkide ifade→exhibit'e, redaksiyonda iki imtiyazlı sayfaya (teslimat kaydı bilerek açık), itirazda sadece leading soruya. Hepsi UYDURMA malzeme kullanır — açık bulmacayı asla çözmez. `useClock` her demo için lokal rAF döngüsüdür, store'a `notify()` basmaz.
- **Zorluk board'ları da ölçekler** (`boardTierOf`, tier 0/1/2; **hard=realistic tek tier**, kullanıcının açık isteği). `chance()` DEĞİŞMEDİ — zar hâlâ zorluktan bağımsız; değişen elinle oynadığın şey: kilit bandı ±4/±3/±2 ve aşınma ×0.78/×1/×1.5, sabotaj hız ×0.9/×1/×1.18 + tolerans +2/0/−2, kronoloji +0/+0/+1 kart, redaksiyon −1/0/+1 sayfa, çelişki +1/0/−1 hak. **İTİRAZ MUAF** (penceresi zaten oyunun en darı) — testte veriyle zorlanır: objection board'u hiç `diff` taşımaz.
- **Kilit sertleşti:** `LOCK_WEAR_LOAD` 0.62→1.05. Kör tarama artık pikin bütçesini yiyor (aramak pahalı, kararlı hamle beceri). `locksim` MEDIUM 43/56/77%, HARD 21/38/39% (SNEAKY 0) — HARD'da SNEAKY ilk kez belirleyici.
- **Save schema v23:** `migrateV22ToV23` açık board'lara `diff:1` damgalar; **tier 1 eski eğriyi birebir üretir**, bu yüzden yarım kalmış bulmaca geçersiz olmaz (regresyon `deepEqual` ile kilitler). Doğrulama `ch.diff`'i tier kümesine karşı zorlar ve yeniden türetmede KAYITTAKİ tier'ı kullanır (Power Cut `rules` grandfather kalıbının aynısı).

**v1.9.30.1 eklendi (2026-08-26, oynanış geri bildirimi):**
- **Rehberler taklit değil GERÇEK board:** `BoardGuide.jsx` artık board'ların kopyasını çizmiyor, `LockpickMinigame`/`PowerCutMinigame`/`TimelineMinigame`/`ContradictionMinigame`/`RedactionMinigame`/`ObjectionMinigame` bileşenlerinin kendisini `demo` prop'uyla çiziyor (doku birebir aynı, tek bakım noktası). `demo` motoru çağıran her handler'ı kapatır ve autofocus'u atlar; challenge nesnesi rehberde UYDURMA malzemeyle kurulur, dosya motoru import ETMEZ.
- **İmleç ölçümle konumlanıyor:** yüzde tabanlı konum silindi (font/yerleşim değişince kayıyordu, hareketli hedefte geriden geliyordu). `center()` + `useLayoutEffect` ile hedef kontrol ölçülür; kilitte topuz ray genişliğinden türetilir. Sapma ~1px.
- **Zamanlı board'lar rehber açıkken DURUR:** itiraz/kilit/sabotaj `paused` prop'u alır, frame döngüsü hiç dönmez; açılışta `checkpointActionChallenge()` donmuş konumu kaydeder. (Önceden yardımı okumak duruşmayı kaybettiriyordu.)
- **Regresyon:** altı board dosyası kaynak düzeyinde taranır (demo prop'u, kapalı handler'lar, `demo||paused` erken dönüşü, overlay'in `paused={guide}` geçmesi, rehberin motoru import etmemesi). Guard'ların bozulunca gerçekten kırdığı ayrıca doğrulandı.

**v1.9.31 eklendi (2026-08-26, kullanıcı isteği — basma hissi, menüye dönüş, itirazın kapsamı):**
- **Demo basma efekti:** `BoardGuide` artık basılan kontrole `guide-pressed` sınıfı veriyor (gerçek `.btn:active` ile aynı 3px iniş + altın halka); demo butonları disabled olduğu için rehber kendi basmasını sürüyor. Her `press:` bir `hit:` selector'ü taşımak ZORUNDA (regresyon zorluyor).
- **SAVE & QUIT (`quitToMenu`/`canQuitToMenu`, SettingsOverlay):** kaydeder → `setS(null)` → başlık ekranı; slot değiştirmek için reload gerekmiyor. IRONMAN muaf (kaydı yok).
- **İtirazın kapsamı:** ölçümle karar verildi — üretilen duruşma dosyalarının %57'sinin sorgu metni YOKTU. `buildExamination` (casegen) her duruşma dosyasına KENDİ isim/belgelerinden metin kuruyor (18 temiz + 22 uygunsuz kalıp, %85 benzersiz). **İfade alma (`objection.depo`)** hakim gerektirmiyor → mahkeme şartı kalktı; UI SUSTAINED→PRESERVED / OVERRULED→SPEAKING OBJ. El yazması `depo` ve `court2` kendi metinlerini aldı. Taşıyan dosya %9.6→%44.4, kariyer başına 3.9→~17.8.
- **İki board yarışmıyor:** aynı dosya hem duruşma hem kronoloji taşıyabilir; `choose()` yazı-tura atar (önce hep itiraz denendiği için kronoloji açlıktan ölüyordu).
- **Fiyat yeniden belirlendi:** 320 kariyerlik soak %69→%55 düştü; bot ortalama **+4.9 kenar** kazandığı için sebep board DEĞİL, saat+yorgunluktu. `OBJECTION_HOURS 0.5→0` (zaten içindeysen ayağa kalkmak saat yakmaz), `OBJECTION_FATIGUE 3→1`. Hazırlık board'ları saatlerini korur. Ölçüm: %65.3 kazanma, medyan gün 21 — eski bant. `validActionChallengeBase(…,minCost=.5)` eklendi; yalnız duruşma tabanı düşürebilir (kaynak-düzeyi guard).

**v1.9.32 eklendi (2026-08-26, kullanıcı isteği — barodan atılma + bağlamsal ses):**
- **Barodan atılma, GİZLİ ısı (`ethics.js`, saf modül — `fraud.js` deseni):** `S.barHeat` HİÇBİR YERDE görünmez (regresyon beş bileşen dosyasını tarar). Besleyenler: yakalanma 24, belge gizleme yaptırımı 15, hakim rüşveti 8 (ödemek suçtur, işe yaraması alakasız); gece −1 azalır. Mektuplar tek geri bildirim kanalıdır: GRIEVANCE@26 → COMPLAINT@54 → HEARING@80, her aşamada %100'lük çıkış var, yalnız 3. aşamanın pervasız kolu `gameOver("DISBARRED")`. **Aşama atlanmaz** (`Math.min(reached,stage+1)`) — gizli ısıda mektup atlamak uyarı silmektir.
- **Fraud çakışması:** Fraud'da baro mektubu AÇILMAZ; ilgi `fraudRisk.suspicion`'a yazılır (iki paralel kimlik soruşturması yerine mevcut merdiven hızlanır).
- **Save schema v24** + `migrateV23ToV24` (eski kariyerler temiz başlar). Doğrulama: sayaç/aşama tutarlılığı, pending gün eşlemesi ve **`heat ≤ ihlallerin üretebileceği maksimum`** (test bu açığı buldu).
- **Bağlamsal ses (`sound.js`):** `setRoomTone` dört oda — `office` / `court` (duruşma+çelişki board'u: kesim 430, tempo yavaş) / `afterhours` (saat bitti veya mesai: tıslama artar) / `spent` (FATIGUE≥75: kesim 300, detune 14). Yorgunluk hepsini ezer. `App.jsx` her render'da `refreshRoomTone()` çağırır (idempotent). **Olay ölçeği:** dosya hükümlerinde `SFX.win(scale)`/`SFX.lose(scale)`, scale = rütbe×.65 + tier×.35; board tıkırtıları ölçeklenmez.

**v1.9.33 eklendi (2026-08-27, kullanıcı isteği — DURUŞMA tur 1/3):**
- **`trial.js` (saf modül):** jüri ikna oranı SADECE modelde; `TrialOverlay.jsx` onu yazmıyor da hesaplamıyor da (regresyon dosyayı tarar). Başlangıç dosyanın `strength`'inden gelir, salınım açılış ±6-8 / argüman ±5-7 / kapanış ±7-10 / tutan itiraz +4 / yersiz −3 / **kaçırılan geçersiz argüman −6**; tavan `JURY_MAX`(85), taban 10. **Hüküm = çubuğun kendisi**, ikinci gizli kontrol yok. Geri bildirim yalnız `roomLine()` anlatısı.
- **Okumalı itiraz:** 8'li sabit gerekçe listesi; doğru gerekçe SUSTAINED, yanlış gerekçe/temiz argümana itiraz OVERRULED. Temiz argümana susmak bedelsizdir. Gerçek zamanlı board duruşma DIŞINDA ve ifade almalarda kalır.
- **Uzlaşma:** karşı taraf `OFFER_AT`(58) üstünde BİR kez teklif eder; ret `offerFloor=jury+12` yapar (gizli çubuğu bedava yoklamayı kapatır).
- **Maliyet:** `TRIAL_HOURS=[4,5,6]` aşama sayısına göre + ağır yorgunluk; ödül ×2, ceza ×1.5; devredilemez. Dosyada seçenek `style:"trial"`, `chance()` **null** döner (sahte oran üretmez).
- **Save schema v25** + `migrateV24ToV25`; duruşma ve dosyası ayrılamaz (yetim `trialInProgress` reddedilir). Soak kanonik snapshot'ına `trial`/`trialResult`/`barHeat` eklendi ve bot duruşma oynamayı öğrendi (aksi halde 498 integrity hatası).
